...
SELECT player,teamid, stadium, mdate
  FROM game JOIN goal ON (goal.teamid = 'GER' AND id=matchid))
...
SELECT team1, team2, player
  FROM game JOIN goal ON (goal.player LIKE 'Mario%' AND id=matchid)
...
SELECT mdate, teamname
 FROM game JOIN eteam ON (eteam.coach LIKE 'Fernando Santos' AND team1 = eteam.id)

-- 7. List the player for every goal scored in a game where the stadium was 'National Stadium, Warsaw'

SELECT player
 FROM game JOIN goal ON (stadium LIKE 'National Stadium, Warsaw' AND matchid = id)
...
-- 9. Show teamname and the total number of goals scored. COUNT and GROUP BY. You should COUNT(*) in the SELECT line and GROUP BY teamname.
